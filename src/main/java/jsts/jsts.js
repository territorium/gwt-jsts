...
TreeMap.prototype.fixAfterInsertion = function (x) {
  ...
  while (x != null && x != this.root_ && x.parent.color == RED) {
    if (parentOf(x) == leftOf$1(parentOf(parentOf(x)))) {
      var y = rightOf$1(parentOf(parentOf(x)));
      if (colorOf(y) == RED) {
      ...
      var y = leftOf$1(parentOf(parentOf(x)));
      if (colorOf(y) == RED) {
        setColor(parentOf(x), BLACK);
        setColor(y, BLACK);
        setColor(parentOf(parentOf(x)), RED);
...
inherits(GeometryCollection$1, Geometry);
extend(GeometryCollection$1.prototype, {
	...
	copy: function copy() {
		...
		return new GeometryCollection$1(geometries, this.factory);
	},
	...
	getClass: function getClass() {
		return GeometryCollection$1;
	}
});
GeometryCollection$1.serialVersionUID = -5694727726395021467;

function MultiLineString$1() {
	var lineStrings = arguments[0],
	    factory = arguments[1];
	GeometryCollection$1.call(this, lineStrings, factory);
}
inherits(MultiLineString$1, GeometryCollection$1);
extend(MultiLineString$1.prototype, {
	getSortIndex: function getSortIndex() {
	...
	copy: function copy() {
		...
		return new MultiLineString$1(lineStrings, this.factory);
	},
	interfaces_: function interfaces_() {
		return [Lineal];
	},
...
MultiLineString$1.serialVersionUID = 8166665132445433741;
...
extend(BoundaryOp.prototype, {
	...
	getBoundary: function getBoundary() {
		if (this.geom instanceof LineString$1) return this.boundaryLineString(this.geom);
		if (this.geom instanceof MultiLineString$1) return this.boundaryMultiLineString(this.geom);
...
inherits(LineString$1, Geometry);
extend(LineString$1.prototype, {
	computeEnvelopeInternal: function computeEnvelopeInternal() {
		if (this.isEmpty()) {
			return new Envelope();
		}
	...
	isEquivalentClass: function isEquivalentClass(other) {
		return other instanceof LineString$1;
	},
	clone: function clone() {
	...
	copy: function copy() {
		return new LineString$1(this.points.copy(), this.factory);
	},
...
LineString$1.serialVersionUID = 3110669828065365560;
...
inherits(Point$1, Geometry);
extend(Point$1.prototype, {
	computeEnvelopeInternal: function computeEnvelopeInternal() {
		if (this.isEmpty()) {
			return new Envelope();
		}
	...
	copy: function copy() {
		return new Point$1(this.coordinates.copy(), this.factory);
	},
	getCoordinateSequence: function getCoordinateSequence() {
...
Point$1.serialVersionUID = 4902022702746614570;
...
inherits(Polygon$1, Geometry);
extend(Polygon$1.prototype, {
	computeEnvelopeInternal: function computeEnvelopeInternal() {
	...
	copy: function copy() {
		...
		return new Polygon$1(shell, holes, this.factory);
	},
...
Polygon$1.serialVersionUID = -3494792200821764533;

function MultiPoint$1() {
	var points = arguments[0],
	    factory = arguments[1];
	GeometryCollection$1.call(this, points, factory);
}
inherits(MultiPoint$1, GeometryCollection$1);
extend(MultiPoint$1.prototype, {
	getSortIndex: function getSortIndex() {
	...
	copy: function copy() {
		...
		return new MultiPoint$1(points, this.factory);
	},
	interfaces_: function interfaces_() {
		return [Puntal];
	},
...
MultiPoint$1.serialVersionUID = -8048474874175355449;
...
function LinearRing() {
	if (arguments[0] instanceof Coordinate && arguments[1] instanceof GeometryFactory) {
		...
		LineString$1.call(this, _points, _factory);
		this.validateConstruction();
	}
}
inherits(LinearRing, LineString$1);
...
function MultiPolygon$1() {
	var polygons = arguments[0],
	    factory = arguments[1];
	GeometryCollection$1.call(this, polygons, factory);
}
inherits(MultiPolygon$1, GeometryCollection$1);
extend(MultiPolygon$1.prototype, {
	getSortIndex: function getSortIndex() {
	...
	copy: function copy() {
		...
		return new MultiPolygon$1(polygons, this.factory);
	},
	interfaces_: function interfaces_() {
		return [Polygonal];
	},
...
MultiPolygon$1.serialVersionUID = -551033529766975875;
...
extend(GeometryEditor.prototype, {
	...
	editGeometryCollection: function editGeometryCollection(collection, operation) {
		...
		if (collectionForType.getClass() === MultiPoint$1) {
			return this.factory.createMultiPoint(geometries.toArray([]));
		}
		if (collectionForType.getClass() === MultiLineString$1) {
			return this.factory.createMultiLineString(geometries.toArray([]));
		}
		if (collectionForType.getClass() === MultiPolygon$1) {
...
extend(GeometryFactory.prototype, {
	...
	createLineString: function createLineString() {
		if (arguments.length === 0) {
			...
			if (arguments[0] instanceof Array) {
				...
				var _coordinates = arguments[0];
				return new LineString$1(_coordinates, this);
			}
		}
	},
	createMultiLineString: function createMultiLineString() {
		if (arguments.length === 0) {
			return new MultiLineString$1(null, this);
		} else if (arguments.length === 1) {
			var lineStrings = arguments[0];
			return new MultiLineString$1(lineStrings, this);
		}
	},
	...
	createPolygon: function createPolygon() {
		if (arguments.length === 0) {
			...
			var _shell = arguments[0],
			    holes = arguments[1];
			return new Polygon$1(_shell, holes, this);
		}
	},
	getSRID: function getSRID() {
		return this.SRID;
	...
	createGeometryCollection: function createGeometryCollection() {
		if (arguments.length === 0) {
			return new GeometryCollection$1(null, this);
		} else if (arguments.length === 1) {
			var geometries = arguments[0];
			return new GeometryCollection$1(geometries, this);
		}
	},
	...
	createMultiPolygon: function createMultiPolygon() {
		if (arguments.length === 0) {
			return new MultiPolygon$1(null, this);
		} else if (arguments.length === 1) {
			var polygons = arguments[0];
			return new MultiPolygon$1(polygons, this);
		}
	},
	createMultiPoint: function createMultiPoint() {
		if (arguments.length === 0) {
			return new MultiPoint$1(null, this);
		} else if (arguments.length === 1) {
			if (arguments[0] instanceof Array) {
				var point = arguments[0];
				return new MultiPoint$1(point, this);
			} else if (arguments[0] instanceof Array) {
				var coordinates = arguments[0];
				return this.createMultiPoint(coordinates !== null ? this.getCoordinateSequenceFactory().create(coordinates) : null);
			} else if (hasInterface(arguments[0], CoordinateSequence)) {
...
var extract$1 = {
  ...
  point: function point(_point) {
    return extract$1.coordinate.call(this, _point.coordinates.coordinates[0]);
  },
  ...
  multipoint: function multipoint(_multipoint) {
    ...
    for (var i = 0, len = _multipoint.geometries.length; i < len; ++i) {
      array.push('(' + extract$1.point.apply(this, [_multipoint.geometries[i]]) + ')');
    }
    return array.join(',');
  },
  ...
  linestring: function linestring(_linestring) {
    ...
    for (var i = 0, len = _linestring.points.coordinates.length; i < len; ++i) {
      array.push(extract$1.coordinate.apply(this, [_linestring.points.coordinates[i]]));
    }
    return array.join(',');
  },
  linearring: function linearring(_linearring) {
    ...
    for (var i = 0, len = _linearring.points.coordinates.length; i < len; ++i) {
      array.push(extract$1.coordinate.apply(this, [_linearring.points.coordinates[i]]));
    }
    return array.join(',');
  },
  ...
  multilinestring: function multilinestring(_multilinestring) {
    ...
    for (var i = 0, len = _multilinestring.geometries.length; i < len; ++i) {
      array.push('(' + extract$1.linestring.apply(this, [_multilinestring.geometries[i]]) + ')');
    }
    return array.join(',');
  },
  ...
  polygon: function polygon(_polygon) {
    var array = [];
    array.push('(' + extract$1.linestring.apply(this, [_polygon.shell]) + ')');
    for (var i = 0, len = _polygon.holes.length; i < len; ++i) {
      array.push('(' + extract$1.linestring.apply(this, [_polygon.holes[i]]) + ')');
    }
    return array.join(',');
  },
  ...
  multipolygon: function multipolygon(_multipolygon) {
    ...
    for (var i = 0, len = _multipolygon.geometries.length; i < len; ++i) {
      array.push('(' + extract$1.polygon.apply(this, [_multipolygon.geometries[i]]) + ')');
    }
    return array.join(',');
  },
...
extend(Centroid.prototype, {
	...
	add: function add() {
		if (arguments[0] instanceof Polygon$1) {
			...
			} else if (geom instanceof LineString$1) {
...
extend(GeometryTransformer.prototype, {
	...
	transform: function transform(inputGeom) {
		...
		if (inputGeom instanceof MultiPoint$1) return this.transformMultiPoint(inputGeom, null);
		if (inputGeom instanceof LinearRing) return this.transformLinearRing(inputGeom, null);
		if (inputGeom instanceof LineString$1) return this.transformLineString(inputGeom, null);
		if (inputGeom instanceof MultiLineString$1) return this.transformMultiLineString(inputGeom, null);
		if (inputGeom instanceof Polygon$1) return this.transformPolygon(inputGeom, null);
		if (inputGeom instanceof MultiPolygon$1) return this.transformMultiPolygon(inputGeom, null);
		if (inputGeom instanceof GeometryCollection$1) return this.transformGeometryCollection(inputGeom, null);
...
extend(GeometryCollectionIterator.prototype, {
	next: function next() {
		...
		if (obj instanceof GeometryCollection$1) {
...
GeometryCollectionIterator.isAtomic = function (geom) {
	return !(geom instanceof GeometryCollection$1);
};
...
extend(PointLocator.prototype, {
	...
	computeLocation: function computeLocation(p, geom) {
		...
		if (geom instanceof LineString$1) {
			...
			this.updateLocationInfo(this.locateInternal(p, geom));
		} else if (geom instanceof MultiLineString$1) {
			var ml = geom;
			for (var i = 0; i < ml.getNumGeometries(); i++) {
				var l = ml.getGeometryN(i);
				this.updateLocationInfo(this.locateInternal(p, l));
			}
		} else if (geom instanceof MultiPolygon$1) {
			var mpoly = geom;
...
extend(LinearComponentExtracter.prototype, {
	filter: function filter(geom) {
		...
		if (geom instanceof LineString$1) this.lines.add(geom);
	},
	...
	if (arguments.length === 1) {
		...
		if (hasInterface(arguments[0], Collection) && hasInterface(arguments[1], Collection)) {
			...
			var _geom3 = arguments[0],
			    _lines3 = arguments[1];
			if (_geom3 instanceof LineString$1) {
				_lines3.add(_geom3);
			} else {
...
extend(GeometryGraph.prototype, {
	...
	computeSelfNodes: function computeSelfNodes() {
		if (arguments.length === 2) {
			...
			var isRings = this.parentGeom instanceof LinearRing || this.parentGeom instanceof Polygon$1 || this.parentGeom instanceof MultiPolygon$1;
	...
	add: function add() {
		if (arguments.length === 1) {
			...
			if (g instanceof MultiPolygon$1) this.useBoundaryDeterminationRule = false;
			if (g instanceof Polygon$1) this.addPolygon(g);else if (g instanceof LineString$1) this.addLineString(g);else if (g instanceof Point$1) this.addPoint(g);else if (g instanceof MultiPoint$1) this.addCollection(g);else if (g instanceof MultiLineString$1) this.addCollection(g);else if (g instanceof MultiPolygon$1) this.addCollection(g);else if (g instanceof GeometryCollection$1) this.addCollection(g);else throw new UnsupportedOperationException(g.getClass().getName());
...
extend(InteriorPointArea.prototype, {
	...
	widestGeometry: function widestGeometry() {
		if (arguments[0] instanceof GeometryCollection$1) {
			var gc = arguments[0];
			if (gc.isEmpty()) {
				return gc;
			}
...
extend(MinimumDiameter.prototype, {
	...
	computeWidthConvex: function computeWidthConvex(convexGeom) {
		if (convexGeom instanceof Polygon$1) this.convexHullPts = convexGeom.getExteriorRing().getCoordinates();else this.convexHullPts = convexGeom.getCoordinates();
...
extend(DensifyTransformer.prototype, {
	...
	transformPolygon: function transformPolygon(geom, parent) {
		...
		if (parent instanceof MultiPolygon$1) {
			return roughGeom;
		}
		return this.createValidArea(roughGeom);
	},
	transformCoordinates: function transformCoordinates(coords, parent) {
		...
		if (parent instanceof LineString$1 && newPts.length === 1) {
...
extend(LineDissolver.prototype, {
	...
	add: function add() {
		if (arguments[0] instanceof Geometry) {
			...
			_geometry.apply({
				...
				filter: function filter(component) {
					if (component instanceof LineString$1) {
						this.add(component);
					}
				}
			});
		...
		} else if (arguments[0] instanceof LineString$1) {
...
var extract$2 = {
  ...
  Polygon: function Polygon(polygon) {
    var array = [];
    var shellGeoJson = extract$2.LineString.apply(this, [polygon.shell]);
...
extend(IsSimpleOp.prototype, {
	...
	computeSimple: function computeSimple(geom) {
		this.nonSimpleLocation = null;
		if (geom.isEmpty()) return true;
		if (geom instanceof LineString$1) return this.isSimpleLinearGeometry(geom);
		if (geom instanceof MultiLineString$1) return this.isSimpleLinearGeometry(geom);
		if (geom instanceof MultiPoint$1) return this.isSimpleMultiPoint(geom);
		if (hasInterface(geom, Polygonal)) return this.isSimplePolygonal(geom);
		if (geom instanceof GeometryCollection$1) return this.isSimpleGeometryCollection(geom);
		return true;
	},
	isSimple: function isSimple() {
		this.nonSimpleLocation = null;
...
extend(PolygonExtracter.prototype, {
	filter: function filter(geom) {
		if (geom instanceof Polygon$1) this.comps.add(geom);
	},
	interfaces_: function interfaces_() {
		return [GeometryFilter];
	},
...
PolygonExtracter.getPolygons = function () {
	if (arguments.length === 1) {
		...
		var _geom = arguments[0],
		    list = arguments[1];
		if (_geom instanceof Polygon$1) {
			list.add(_geom);
		} else if (_geom instanceof GeometryCollection$1) {
...
extend(PointExtracter.prototype, {
	filter: function filter(geom) {
		if (geom instanceof Point$1) this.pts.add(geom);
	},
	interfaces_: function interfaces_() {
		return [GeometryFilter];
	},
...
extend(ConnectedElementLocationFilter.prototype, {
	filter: function filter(geom) {
		if (geom instanceof Point$1 || geom instanceof LineString$1 || geom instanceof Polygon$1) this.locations.add(new GeometryLocation(geom, 0, geom.getCoordinate()));
	},
	interfaces_: function interfaces_() {
		return [GeometryFilter];
	},
...
extend(DistanceOp.prototype, {
	...
	computeMinDistance: function computeMinDistance() {
		if (arguments.length === 0) {
			...
			if (arguments[2] instanceof Array && arguments[0] instanceof LineString$1 && arguments[1] instanceof Point$1) {
				var line = arguments[0],
				    pt = arguments[1],
				    locGeom = arguments[2];
...
extend(ConnectedInteriorTester.prototype, {
	...
	visitShellInteriors: function visitShellInteriors(g, graph) {
		...
		if (g instanceof MultiPolygon$1) {
			var mp = g;
			for (var i = 0; i < mp.getNumGeometries(); i++) {
...
extend(IsValidOp.prototype, {
	checkInvalidCoordinates: function checkInvalidCoordinates() {
		...
		} else if (arguments[0] instanceof Polygon$1) {
			var poly = arguments[0];
	...
	checkValid: function checkValid() {
		if (arguments[0] instanceof Point$1) {
		...
		} else if (arguments[0] instanceof MultiPoint$1) {
			var _g2 = arguments[0];
		...
		} else if (arguments[0] instanceof MultiPolygon$1) {
			var _g6 = arguments[0];
			...
			if (this.validErr !== null) return null;
			this.checkConnectedInteriors(graph);
		} else if (arguments[0] instanceof GeometryCollection$1) {
			var gc = arguments[0];
			for (var i = 0; i < gc.getNumGeometries(); i++) {
				var g = gc.getGeometryN(i);
				this.checkValid(g);
			...
			if (_g7 instanceof Point$1) this.checkValid(_g7);else if (_g7 instanceof MultiPoint$1) this.checkValid(_g7);else if (_g7 instanceof LinearRing) this.checkValid(_g7);else if (_g7 instanceof LineString$1) this.checkValid(_g7);else if (_g7 instanceof Polygon$1) this.checkValid(_g7);else if (_g7 instanceof MultiPolygon$1) this.checkValid(_g7);else if (_g7 instanceof GeometryCollection$1) this.checkValid(_g7);else throw new UnsupportedOperationException(_g7.getClass().getName());
		}
	},
...
extend(LineStringAdder.prototype, {
	filter: function filter(g) {
		if (g instanceof LineString$1) this.p.add(g);
	},
	interfaces_: function interfaces_() {
		return [GeometryComponentFilter];
	},
...
var polygonize$1 = Object.freeze({
...
extend(RectangleContains.prototype, {
	isContainedInBoundary: function isContainedInBoundary(geom) {
		if (geom instanceof Polygon$1) return false;
		if (geom instanceof Point$1) return this.isPointContainedInBoundary(geom);
		if (geom instanceof LineString$1) return this.isLineStringContainedInBoundary(geom);
		for (var i = 0; i < geom.getNumGeometries(); i++) {
			var comp = geom.getGeometryN(i);
...
extend(ShortCircuitedGeometryVisitor.prototype, {
	applyTo: function applyTo(geom) {
		for (var i = 0; i < geom.getNumGeometries() && !this._isDone; i++) {
			...
			if (!(element instanceof GeometryCollection$1)) {
				this.visit(element);
				if (this.isDone()) {
					this._isDone = true;
					return null;
...
extend(GeometryContainsPointVisitor.prototype, {
	...
	visit: function visit(geom) {
		if (!(geom instanceof Polygon$1)) return null;
...
extend(PrecisionReducerCoordinateOperation.prototype, {
	editCoordinates: function editCoordinates(coordinates, geom) {
		...
		var minLength = 0;
		if (geom instanceof LineString$1) minLength = 2;
...
extend(DPTransformer.prototype, {
	...
	transformLinearRing: function transformLinearRing(geom, parent) {
		var removeDegenerateRings = parent instanceof Polygon$1;
...
extend(LineStringTransformer.prototype, {
	transformCoordinates: function transformCoordinates(coords, parent) {
		...
		if (parent instanceof LineString$1) {
